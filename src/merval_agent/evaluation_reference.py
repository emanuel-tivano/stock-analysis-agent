...
def _reference(bars):
    ...
    result = {
        ...
        if bars and all(b.volume is not None for b in bars)
        else None,
...
def compare_metrics(actual: dict, expected: dict) -> dict:
    ...
    return {
        k: {
            "actual": actual.get(k),
            "reference": v,
            "absolute_error": abs(actual[k] - v)
            if v is not None and actual.get(k) is not None
            else None,
            "pass": actual.get(k) is None
            if v is None
            else (actual.get(k) is not None and isclose(actual[k], v, rel_tol=1e-10, abs_tol=1e-9)),
        }
